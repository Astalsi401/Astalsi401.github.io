const Calculator = () => {
  ...
  return (
    <form className="my-3 mx-auto calculator" onChange={tuneformula}>
      <fieldset className="border-0">
        <Label label="重量比" name="ratio" type="number" step="0.01" placeholder="0~1間的小數" value={status.ratio} />
        {status.ratio > 1 || status.ratio < 0 ? <div className="text-warn">請輸入0~1間的小數</div> : null}
        <Label label="最大值" name="max" type="number" value={status.max} />
        <Label label="最小值" name="min" type="number" value={status.min} />
        <label className="my-2 w-100">
          <div>驅動方式：</div>
          <select name="type" value={status.type}>
...
const Content = () => {
  ...
  return sections.map((section) => <Block id={section.id && section.id} className={section.class && section.class} title={section.title} content={section.content} />);
};
