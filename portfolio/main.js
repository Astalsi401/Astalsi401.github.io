        ...
        {index.pages.map((page) => (
          <div className="col-sm-6 col-md-4 col-lg-3 p-2" key={page.page}>
            <a className="d-block bg-white shadow-sm w-100 h-100 text-center text-decoration-none portfolio" href={page.href} target="_blank">
              <div className={`w-100 ratio-16by9 portfolioThumb overflow-hidden ${logo[page.thumbnail] ? "page-logo" : "page-view"}`} style={{ backgroundImage: `url(${logo[page.thumbnail] ? logo[page.thumbnail] : page.thumbnail})` }}>
                <div className="tags">
                  {page.tags.map((tag, i) => (
